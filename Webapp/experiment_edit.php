...
if (!can_edit_experiment($_SESSION['role'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        header('Location: experiments.php?updated=1');
        exit;
    }
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Edit Experiment — Research Group DB</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .banner.success { background: #ccffcc; }
...
<body>
  ...
  <div class="ie-window">
    <div class="title-bar">
      <div class="title-bar-text">✏️ Edit Experiment: Research Group Database — Microsoft Internet Explorer</div>
      <div class="title-bar-controls">
        <div class="win-btn">_</div>
        <div class="win-btn">□</div>
        <a href="experiments.php" class="win-btn">✕</a>
      </div>
    </div>
    ...
    <div class="address-bar">
      ...
      <input class="address-input" type="text" value="http://localhost/researchdb/experiment_edit.php?id=<?php echo $id; ?>" readonly>
      <button class="go-btn">Go</button>
    </div>

    <div class="ie-content">
      <div class="page-header">
        <h1>✏️ Edit Experiment #<?php echo $id; ?></h1>
      ...
      <div class="banner success">
        ...
        <div><strong>Saved successfully.</strong> The experiment record has been updated.</div>
      </div>
      <?php endif; ?>
      ...
      <form method="POST" action="experiment_edit.php?id=<?php echo $id; ?>">
        <input type="hidden" name="id" value="<?php echo $id; ?>">

        <div class="form-panel">
          <div class="form-panel-title">Experiment Details</div>

          <div class="form-row">
            <label for="title">Title: *</label>
            <input type="text" id="title" name="title" required
                   value="<?php echo htmlspecialchars($experiment['title']); ?>">
          </div>

          <div class="form-row">
            <label for="objective">Objective:</label>
            <textarea id="objective" name="objective"><?php echo htmlspecialchars($experiment['objective'] ?? ''); ?></textarea>
          </div>

          <div class="form-row">
            <label for="description">Description:</label>
            <textarea id="description" name="description"><?php echo htmlspecialchars($experiment['description'] ?? ''); ?></textarea>
          </div>

          <div class="form-row">
            <label for="start_date">Start Date:</label>
            <input type="date" id="start_date" name="start_date"
                   value="<?php echo htmlspecialchars($experiment['start_date'] ?? ''); ?>">
          </div>

          <div class="form-row">
            <label for="end_date">End Date:</label>
            <input type="date" id="end_date" name="end_date"
                   value="<?php echo htmlspecialchars($experiment['end_date'] ?? ''); ?>">
          </div>
            ...
            <select id="status" name="status">
              <?php foreach (['Active', 'Completed', 'On Hold', 'Cancelled'] as $s): ?>
                <option value="<?php echo $s; ?>"
                  <?php echo $experiment['status'] === $s ? 'selected' : ''; ?>>
                  <?php echo $s; ?>
                </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="btn-row">
          ...
          <a href="experiments.php" class="win98-btn">Cancel</a>
        </div>
  ...
  <div class="taskbar">
    ...
    <div class="taskbar-active">✏️ Edit Experiment — Research Group Database</div>
