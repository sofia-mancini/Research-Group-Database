...
$groupRows = pdo($pdo,
    "SELECT rg.group_id, rg.name, rg.focus, pg.role, pg.join_date
     FROM research_group rg
     JOIN person_group pg ON rg.group_id = pg.group_id
     WHERE pg.person_id = :id
     ORDER BY rg.name",
    ['id' => $id]
)->fetchAll();

$taskRows = pdo($pdo,
    "SELECT t.task_id, t.title, t.status, t.progress, t.due_date
     FROM task t
...
<head>
  <meta charset="UTF-8">
  <title><?php echo htmlspecialchars($member['name']); ?> — Research Group DB</title>
  <style>
...
<body>
  ...
  <div class="ie-window">
    <div class="title-bar">
      <div class="title-bar-text">👤 <?php echo htmlspecialchars($member['name']); ?> — Research Group Database — Microsoft Internet Explorer</div>
    ...
    <div class="address-bar">
      ...
      <input class="address-input" type="text" value="http://localhost/researchdb/member_view.php?id=<?php echo $id; ?>" readonly>
    ...
    <div class="ie-content">
      ...
      <div class="quick-links">
        ...
        <a class="quick-link-btn" href="members.php" style="border-top:2px solid #808080;border-left:2px solid #808080;border-right:2px solid #fff;border-bottom:2px solid #fff;">👥 Members</a>
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px;">
        <a class="toolbar-action" href="members.php">◀ Back to Members</a>
        <?php if (can_edit_person($_SESSION['role'])): ?>
          <a class="toolbar-action" href="member_edit.php?id=<?php echo $id; ?>">✏️ Edit Member</a>
        <?php endif; ?>
        <?php if (can_delete($_SESSION['role'])): ?>
          <a class="toolbar-action" href="member_delete.php?id=<?php echo $id; ?>"
             onclick="return confirm('Delete this member?')" style="color:#990000;">🗑️ Delete</a>
        <?php endif; ?>
      </div>

      <div class="main-panel">
        <div class="main-panel-header">👤 Member Details</div>
        ...
        <div class="details-section">
          <table class="details-table">
            <tr>
              ...
              <td colspan="3"><?php echo htmlspecialchars($member['email']); ?></td>
            </tr>
            <tr>
              <td class="label">Role:</td>
              <td colspan="5"><span class="role-badge"><?php echo htmlspecialchars($member['role']); ?></span></td>
            </tr>
          </table>
        </div>
        ...
        <div class="subtables-row">
          ...
          <div class="subtable-col">
            <div class="subtable-header">🏛️ Departments</div>
            <div class="subtable-body">
              <?php if (empty($deptRows)): ?>
                <p class="empty-notice">No department assigned.</p>
              <?php else: ?>
              <table>
          ...
          <!-- Research Groups -->
          <div class="subtable-col">
            <div class="subtable-header">🔬 Research Groups</div>
            <div class="subtable-body">
              <?php if (empty($groupRows)): ?>
                <p class="empty-notice">No research groups.</p>
              <?php else: ?>
              <table>
                ...
                <tbody>
                  ...
                  <tr>
                    <td title="<?php echo htmlspecialchars($g['focus']); ?>"><?php echo htmlspecialchars($g['name']); ?></td>
                    <td><?php echo htmlspecialchars($g['role']); ?></td>
                    <td><?php echo htmlspecialchars($g['join_date'] ?? '—'); ?></td>
                  </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
          ...
          <!-- Assigned Tasks -->
          <div class="subtable-col">
            <div class="subtable-header">✅ Assigned Tasks</div>
            <div class="subtable-body">
              <?php if (empty($taskRows)): ?>
                <p class="empty-notice">No tasks assigned.</p>
              <?php else: ?>
              <table>
  ...
  <div class="taskbar">
    ...
    <div class="taskbar-active">👤 <?php echo htmlspecialchars($member['name']); ?> — Research Group Database</div>
