...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    ...
    pdo($pdo, "DELETE FROM expt_literature WHERE lit_id = :id",    ['id' => $id]);
    pdo($pdo, "DELETE FROM project_literature WHERE lit_id = :id", ['id' => $id]);
    pdo($pdo, "DELETE FROM literature WHERE lit_id = :id",         ['id' => $id]);
    header('Location: literature.php?deleted=1');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
  <meta charset="UTF-8">
  <title>Delete Literature — Research Group DB</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .record-preview .title-text { font-weight: bold; word-break: break-word; }
    ...
    <div class="window-body">
      <div class="dialog-row">
        ...
        <div class="dialog-text">
          Are you sure you want to permanently delete this literature entry?
          <br><br>
          This will also remove all linked records in
          <strong>expt_literature</strong> and
          <strong>project_literature</strong>.
          <br>
      ...
      <div class="record-preview">
        <div><span class="label">ID: </span><?php echo htmlspecialchars($lit['lit_id']); ?></div>
        <div><span class="label">Title: </span><span class="title-text"><?php echo htmlspecialchars($lit['title']); ?></span></div>
        <div><span class="label">Year: </span><?php echo htmlspecialchars($lit['year']); ?></div>
        <div><span class="label">Journal: </span><?php echo htmlspecialchars($lit['journal'] ?? '—'); ?></div>
        <?php if ($lit['doi']): ?>
        <div><span class="label">DOI: </span><?php echo htmlspecialchars($lit['doi']); ?></div>
        <?php endif; ?>
      </div>

      <hr class="divider">

      <form method="POST" action="literature_delete.php?id=<?php echo $id; ?>">
