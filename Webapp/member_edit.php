...
// Admin only
if (!can_edit_person($_SESSION['role'])) {
...
$departments = pdo($pdo, "SELECT * FROM department ORDER BY name ASC")->fetchAll();

// Fetch current department assignment
$current_dept = pdo($pdo,
    "SELECT department_id FROM person_dept WHERE person_id = :id",
    ['id' => $id]
)->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $valid_roles = [
        ...
        'Lead Data Scientist', 'Lead Engineer', 'Postdoctoral Fellow',
        'Research Associate'
    ];
    ...
    if (empty($name)) {
        ...
        pdo($pdo,
            "UPDATE Person SET name=:name, email=:email, role=:role WHERE ID=:id",
        ...
        if ($department_id) {
            ...
            pdo($pdo,
                ...
                ['person_id' => $id, 'dept_id' => $department_id]
            );
        } else {
            // Clear department if none selected
            pdo($pdo, "DELETE FROM person_dept WHERE person_id = :id", ['id' => $id]);
        }
...
<head>
  <meta charset="UTF-8">
  <title>Edit Member — Research Group DB</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .form-row label { width: 110px; flex-shrink: 0; padding-top: 2px; font-size: 11px; }
    ...
    .form-row input:focus, .form-row select:focus { outline: 1px dotted #000; outline-offset: -2px; }
...
<body>
  ...
  <div class="ie-window">
    <div class="title-bar">
      <div class="title-bar-text">✏️ Edit Member: Research Group Database — Microsoft Internet Explorer</div>
    ...
    <div class="address-bar">
      ...
      <input class="address-input" type="text" value="http://localhost/researchdb/member_edit.php?id=<?php echo $id; ?>" readonly>
      <button class="go-btn">Go</button>
    </div>

    <div class="ie-content">
      <div class="page-header">
        <h1>✏️ Edit Member #<?php echo $id; ?></h1>
      ...
      <form method="POST" action="member_edit.php?id=<?php echo $id; ?>">
            ...
            <select id="role" name="role">
              <?php foreach ([
                ...
                <option value="<?php echo $r; ?>"
                  <?php echo $member['role'] === $r ? 'selected' : ''; ?>>
                  <?php echo $r; ?>
                </option>
              <?php endforeach; ?>
            </select>
            ...
            <select id="department_id" name="department_id">
              ...
              <?php foreach ($departments as $d): ?>
                <option value="<?php echo $d['department_id']; ?>"
                  <?php echo ($current_dept && $current_dept['department_id'] == $d['department_id']) ? 'selected' : ''; ?>>
  ...
  <div class="taskbar">
    ...
    <div class="taskbar-active">✏️ Edit Member — Research Group Database</div>
