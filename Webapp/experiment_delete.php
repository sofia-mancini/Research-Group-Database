...
// Fetch record so we can show its title in the confirmation
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    ...
    pdo($pdo, "DELETE FROM expt_literature WHERE experiment_id = :id", ['id' => $id]);
    pdo($pdo, "DELETE FROM project_expt WHERE experiment_id = :id",    ['id' => $id]);
    pdo($pdo, "DELETE FROM experiment WHERE experiment_id = :id",       ['id' => $id]);
    header('Location: experiments.php?deleted=1');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
  <meta charset="UTF-8">
  <title>Delete Experiment: Research Group DB</title>
  <style>
    ...
    <div class="window-body">
      <div class="dialog-row">
        ...
        <div class="dialog-text">
          ...
          <strong>expt_literature</strong> and <strong>project_expt</strong>.
          <br>
      ...
      <div class="record-preview">
        <div><span class="label">ID: </span><?php echo htmlspecialchars($experiment['experiment_id']); ?></div>
        <div><span class="label">Title: </span><strong><?php echo htmlspecialchars($experiment['title']); ?></strong></div>
        <div><span class="label">Status: </span><?php echo htmlspecialchars($experiment['status']); ?></div>
        <div><span class="label">Dates: </span><?php echo htmlspecialchars($experiment['start_date'] ?? '—'); ?> → <?php echo htmlspecialchars($experiment['end_date'] ?? 'ongoing'); ?></div>
      </div>

      <hr class="divider">

      <form method="POST" action="experiment_delete.php?id=<?php echo $id; ?>">
